...
const TaskForm = ({ handleFormSubmit, setShowForm, isCustom }) => {
  ...
  const [taskName, setTaskName] = useState(
    isCustom ? "" : predefinedLists.bartenderClose.clName
  );

  const [date, setDate] = useState(new Date().toISOString().slice(0, 10));

  const [subtasks, setSubtasks] = useState(
    isCustom ? [""] : predefinedLists.supervisorUtilities.items
  );
  ...
  useEffect(() => {
    ...
    setTaskName(predefinedLists[sectionName].clName);
    ...
    <form
      onSubmit={e => {
        ...
        const section = isCustom
          ? sectionName
          : predefinedLists[sectionName].clName;
